...
@task()
def prep_db(create_table_statement):
    with psycopg.connect(
        "host=localhost port=5432 user=postgres password=root", autocommit=True
    ) as conn:
        ...
        if len(res.fetchall()) == 0:
            ...
            with psycopg.connect(
                "host=localhost port=5432 dbname=housing_price user=postgres password=root"
            ) as conn:
...
def calculate_metrics_postgresql(curr, raw_data, loaded_model, reference_data):
    ...
    current_data['price'] = loaded_model.predict(
        current_data[num_feat].drop(['ID'], axis=1)
    )
    ...
    curr.execute(
        "INSERT INTO housing_metrics n\
        (timestamp, model_drift_score, model_drift_detected, num_drifted_columns, dataset_drift_detected) n\
            VALUES (%s, %s, %s, %s, %s)",
        (
...
def batch_monitoring_backfill(
    raw_data, loaded_model, reference_data, create_table_statement
):
...
def main_task(MLFLOW_TRACKING_URI, EXPERIMENT_NAME, model_registry_name, client):
    ...
    prod_model = client.get_latest_versions(model_registry_name, stages=["Production"])[
        0
    ]
